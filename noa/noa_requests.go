// TODO
/*
* Right now, only reports the weather in the center of chicago. Some ideas:
* Use a sqlite database that stores some locations under various nicknames.
* Write a second command that lets the user view, add, remove nicknames with coordinates.
* Write a flag that lets the user input coordinates.
*
* Another idea, use another web api to get the approximate location from the ip address.
 */
...
func Noa_request(coords []float64) error {
	// In the future, find a way to autodetect coordinates.
	if len(coords) != 2 {
		return errors.New("Coordinates must have exactly 2 elements.")
	}
